...
namespace WxWomanSyncToZPerson
{
    ...
    public partial class Service1 : ServiceBase
    {
        ...
        public Service1()
        {
            Tool.EventLogMethod("", "", "Excute Service1()", DateTime.Now, EventLogEntryType.Information);
            InitializeComponent();
        }
        protected override void OnStart(string[] args)
        {
            Tool.EventLogMethod("", "", "Excute OnStart()", DateTime.Now, EventLogEntryType.Information);
        ...
        {
            Tool.EventLogMethod("", "", "Excute OnTimer()", DateTime.Now, EventLogEntryType.Information);
            var PrimaryID = "";
            var DomainType = "";
            var NeedIdcard = "";
            try
                ...
                try
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            try
                            {
                                ...
                                if (DomainType == "Person" || DomainType == "Husband")
                                {
                                    ...
                                    if (dt2.Rows.Count > 0)
                                    {
                                        ...
                                        if (row2["idcard"].ToString() != "000000000000000000")
                                        {
                                            ...
                                            var Person_Id = "";
                                            var PersonDT = SqlHelper.ExecuteDataSet(SqlHelper.CityConnectionString,"select * from z_person where idcard='" + row2["idcard"].ToString() + "' order by addtime desc").Tables[0];
                                            ...
                                            if (Person_Id != "")
                                            {
                                                ...
                                                z_PersonDAL.Update(null, Z_PersonModel);
                                                Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Person.Update)", IndexTime, EventLogEntryType.SuccessAudit);
                                            }
                                            else
                                            {
                                                ...
                                                string Sec = Tool.SecurityData(Z_PersonModel);
                                                if (Sec != "") { throw new Exception(Sec); }
                                                z_PersonDAL.AddByString(null, Z_PersonModel);
                                                Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Person.Insert)", IndexTime, EventLogEntryType.SuccessAudit);
                                            }
                                            var PersonExDT = SqlHelper.ExecuteDataSet(SqlHelper.CityConnectionString,"select * from z_person_extend where Person_id='" + Person_Id + "'").Tables[0];
                                            if (PersonExDT.Rows.Count > 0)
                                            {
                                                string UpdateSql = Tool.GetUpdateExtend(row2, Person_Id, IndexTime);
                                                SqlHelper.ExecuteNonQuery(SqlHelper.CityConnectionString,UpdateSql);
                                                Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Person_Extend.Update)", IndexTime, EventLogEntryType.SuccessAudit);
                                            }
                                            else
                                            {
                                                string InsertSql = Tool.GetInsertExtend(row2, Person_Id, IndexTime);
                                                SqlHelper.ExecuteNonQuery(SqlHelper.CityConnectionString,InsertSql);
                                                Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Person_Extend.Insert)", IndexTime, EventLogEntryType.SuccessAudit);
                                            }
                                        }
                                    }
                                }
                                    ...
                                    if (dt2.Rows.Count > 0)
                                    {
                                        ...
                                        if (dt3.Rows.Count > 0)
                                        {
                                            ...
                                            if (idcard != "000000000000000000" && idcard != "000000000000000")
                                            {
                                                NeedIdcard = idcard;
                                                var Person_id = "";
                                                var PersonDT = SqlHelper.ExecuteDataSet(SqlHelper.CityConnectionString,"select * from  z_person where idcard='" + idcard + "' order by AddTime desc").Tables[0];
                                                if (PersonDT.Rows.Count > 0)
                                                {
                                                    Person_id = PersonDT.Rows[0]["Person_Id"].ToString();
                                                }
                                                if (Person_id != "")
                                                {
                                                    ...
                                                    UpdateSql.Append("update z_person  set marryId = '" + marryId + "'");
                                                    if (marryDate != "")
                                                    {
                                                    ...
                                                    SqlHelper.ExecuteNonQuery(SqlHelper.CityConnectionString,UpdateSql.ToString());
                                                    Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Person.Update)", IndexTime, EventLogEntryType.SuccessAudit);
                                                }
                                                else
                                                {
                                                ...
                                                else
                                                {
                                                    ...
                                                    InsertSql1.Append("insert into z_person_extend  (PKID,Person_id");
                                                    ...
                                                    SqlHelper.ExecuteNonQuery(SqlHelper.CityConnectionString,Sql);
                                                    Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Person_Extend.Insert)", IndexTime, EventLogEntryType.SuccessAudit);
                                                }
                                            }
                                        }
                                ...
                                else if (DomainType == "Child")
                                {
                                    ...
                                    if (dt1.Rows.Count > 0)
                                    {
                                        ...
                                        {
                                            NeedIdcard = idcard;
                                            var ChildDT = SqlHelper.ExecuteDataSet(SqlHelper.CityConnectionString,"select * from z_child where idcard='" + idcard + "' order by AddTime Desc").Tables[0];
                                            if (ChildDT.Rows.Count > 0)
                                            {
                                                string UpdateSql = Tool.GetUpdateChild(dt1.Rows[0], ChildDT.Rows[0]["PKID"].ToString(), IndexTime);
                                                SqlHelper.ExecuteNonQuery(SqlHelper.CityConnectionString,UpdateSql);
                                                Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Child.Update)", IndexTime, EventLogEntryType.SuccessAudit);
                                            }
                                            else
                                            {
                                                string InsertSql = Tool.GetInsertChild(dt1.Rows[0], Guid.NewGuid().ToString(), IndexTime);
                                                SqlHelper.ExecuteNonQuery(SqlHelper.CityConnectionString,InsertSql);
                                                Tool.EventLogMethod(DomainType, PrimaryID + ":" + NeedIdcard, "同步成功(Z_Child.Insert)", IndexTime, EventLogEntryType.SuccessAudit);
                                            }
                                        }
                                    }
                                }
                            ...
                            {
                                Tool.EventLogMethod(DomainType, PrimaryID, ex.Message, DateTime.Now, EventLogEntryType.Error);
                                continue;
                            }
