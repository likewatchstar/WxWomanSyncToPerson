...
namespace WxWomanSyncToZPerson
{
    ...
    {
        public string LogSource = ConfigurationManager.AppSettings["EventLogSource"].ToString();
        public string LogName = ConfigurationManager.AppSettings["EventLogName"].ToString();
                ...
                using (SqlTransaction tran = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            SqlHelper.ExecuteNonQuery("update WomanSyncToPersonDateTime  set id='" + IndexTime+"'");
                            tran.Commit();
                        }
                    }
                    catch (Exception ex)
                    {
                        Tool.EventLogMethod(null, DomainType, PrimaryID, ex.Message, IndexTime, EventLogEntryType.Error);
                        timer.Start();
                        tran.Rollback();
                    }
                }
